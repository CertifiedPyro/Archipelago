...
def set_all_rules(world: PipWorld) -> None:
    _set_all_entrance_rules(world)
    _set_all_location_rules(world)
    _set_completion_condition(world)


def _set_all_entrance_rules(world: PipWorld) -> None:
    for entrance_name, rule in ENTRANCE_RULES.items():
        world.set_rule(world.get_entrance(entrance_name), rule)


def _set_all_location_rules(world: PipWorld) -> None:
...
def _set_completion_condition(world: PipWorld) -> None:
    # TODO: Set to North Plaza
    end_region = regions.get_room_by_room_label("South Plaza (X-1, Y-6) - ren355 (North)").region_name
